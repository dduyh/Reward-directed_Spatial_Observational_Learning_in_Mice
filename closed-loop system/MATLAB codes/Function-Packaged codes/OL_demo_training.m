...
%%
function [] = OL_demo_training(Folder,s,vid_maze,background,coordinates_in_left,bw_in_left, ...
...
while toc(tStart) < trial_length
    ...
    patch('XData',coordinates_out_left(1,:),'YData',coordinates_out_left(2,:),...
    ...
    %Step 2: Close the left cage door when the Demo mouse is detected leaving the left cage;
    %Or after maximum 1 mins, close the cage door to end the trial;
    if Step_2
        if sum(cage_out_left,'all') < cage_out_thresh
    ...
    elseif Step_4
        if pdist([reward_center ; centroids],'euclidean') < reward_radius
            ...
            if reward_duration > reward_duration_threshold
                ...
                disp('Step 4: Demo mouse is in the reward zone for 2 secs, provide food for the demo mouse    ');
...
%                 pause(2)  % wait for 2 secs
                fprintf(s,'left_door_open/');
                disp('Open the left cage door to let the mouse back to cage     ');
            end
        ...
        %Step 5: Close the left cage door when the demo mouse is detected entering the left cage;
    elseif Step_5
...
save([Data_Folder 'parameters.mat'],'reward_center','reward_radius','duration_threshold','reward_on_maze', ...
    'reward_timepoint','exploration_threshold','coordinates_in_left','coordinates_out_left','T','Speaker_volume');
